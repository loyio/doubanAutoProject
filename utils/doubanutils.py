...
from utils import logfunc
from config import dburls, filepath

logger = logfunc.LogFunc()
group_dict = dict()


def get_cookies(cookie_no):
    # 获取豆瓣登录Cookie信息

    cookies = {}
    with open(filepath.cookie_txt+str(cookie_no)+".txt", "r") as f_cookie:
...
def get_group_joins(user_id):
    ...
    group_joins_url = dburls.DOUBAN_GROUP + "people/" + str(user_id) + "/joins"
...
def group_name_to_id(group_name):
    ...
    if group_name in group_dict:
        ...
        group_url = dburls.DOUBAN_GROUP + group_name
...
def get_verify_code_pic(url, cookie_no):
    # 获取验证码的图片URL和id
...
def get_form_ck_from_cookie(cookie_no):
    # 从cookie中获取ck值（ck: post操作表单隐藏字段）

    douban_cookies = get_cookies(cookie_no)
